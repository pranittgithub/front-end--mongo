  ...
  if (result.success) {
      ...
      <table border="1">
        <thead>
          <tr>
            ...
            <th>Purchase Source</th> <!-- Added Purchase Source Column -->
            <th>Win</th>
            <th>Attempts</th>
            <th>Actions</th>
          </tr>
    ...
    result.users.forEach(user => {
      ...
      row.innerHTML = `
        ...
        <td>${user.purchase}</td> <!-- Display Purchase Source -->
        <td>${user.win}</td>
        <td>${user.attempts}</td>
        <td>
          <button 
...
function convertToCSV(users) {
  const headers = ['Name', 'Email', 'Phone', 'Gender', 'Purchase Source', 'Win', 'Attempts'];
  const rows = users.map(user => [
    user.name,
    user.email,
    user.phone || 'Invalid Phone Number',
    user.gender || 'N/A',  // Added gender to CSV
    user.purchase || 'N/A', // Added purchase to CSV
    user.win,
    user.attempts
  ]);
